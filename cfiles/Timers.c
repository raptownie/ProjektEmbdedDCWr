...
static uint32_t HowLongButtonIsPressed = 0;
...
void TIM1_UP_TIM16_IRQHandler(void){   
   if(((TIM1->SR) & TIM_SR_UIF) == TIM_SR_UIF){      
      LedControl_BlinkLed(LedStructName);      
      UART_DMAHandleUART();      
...
void TIM1_CC_IRQHandler(void){
   if(((TIM1->SR) & TIM_SR_CC1IF) == TIM_SR_CC1IF){            // sprawdzenie czy przerwanie wywolujace funkcje pochodzi od CC1
      UART_Send_Data_Gyro_Accelero_XYZ_UART();
...
void TIM6_DAC_IRQHandler(void){
   if(((TIM6->SR) & TIM_SR_UIF) == TIM_SR_UIF){
      if (MainPerform_CheckMainState(GyroState)) 
         SPI_Gyroskop_L3GD20_XYZ_Read_Calculate();
      else if (MainPerform_CheckMainState(AcceleroState))
         I2C_Accelero_LSM303DLHC_XYZ_Read_Calculate();
      TIM6->SR &= ~TIM_SR_UIF;
   }
}
...
void TIM8_UP_IRQHandler (void){
     if(((TIM8->SR) & TIM_SR_UIF) == TIM_SR_UIF){      
      LedControl_DisplayShow(LedStructName);      
      TIM8->SR &= ~TIM_SR_UIF;
   }
}
